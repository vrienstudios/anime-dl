        ...
        public void BeginExecution()
        {
            ...
            if (args.d)
            {
                if (args.term.IsValidUri())
                {
                    ...
                    ex = args.l
                        ? args.export + Path.DirectorySeparatorChar + manga.metaData.name + ".adl"
                        : Directory.GetCurrentDirectory() +
                          $"{Path.DirectorySeparatorChar}Epubs{Path.DirectorySeparatorChar}" + manga.metaData.name +
                          ".adl";
                ...
                {
                    ArraySegment<MangaChapter> mg = new ArraySegment<MangaChapter>(b, manga.Chapters.Length,
                        b.Length - manga.Chapters.Length);
            ...
            else
            {
                ...
                if (args.term.IsValidUri())
                {
                    ...
                    ex = args.l
                        ? args.export + Path.DirectorySeparatorChar + mdata.name + ".adl"
                        : Directory.GetCurrentDirectory() +
                          $"{Path.DirectorySeparatorChar}Epubs{Path.DirectorySeparatorChar}" + mdata.name + ".adl";
                }
                else
            ...
            if (args.e)
            {
                ...
                manga.ExportToEpub(
                    args.l
                        ? args.export + Path.DirectorySeparatorChar + manga.metaData.name
                        : Directory.GetCurrentDirectory() +
                          $"{Path.DirectorySeparatorChar}Epubs{Path.DirectorySeparatorChar}" + manga.metaData.name,
                    ref am.zapive);
