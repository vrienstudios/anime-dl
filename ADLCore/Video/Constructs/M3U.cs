...
namespace ADLCore.Video.Constructs
{
    ...
    public class M3U
    {
            ...
            string bPath = null, bool mp4 = false, M3UMP4_SETTINGS settings = null)
        {
            ...
            HLSListObject obj;
            if (m3u8Info[0][0] == '#')
                obj = new HLSListObject(m3u8Info);
            else
                obj = new HLSListObject(webClient.DownloadString(m3u8Info[0]).Split('\n'));

             var highestResMedia = obj.dict[(from key in obj.dict.Keys.Where(x => x.Contains("EXT-X-STREAM-INF"))
                 group obj.dict[key] by int.Parse(obj.dict[key]["RESOLUTION"].Split('x')[0]) 
                 into grp1 
                 select new { PARENT = grp1.Max()["PARENT"] }).Last().PARENT];
